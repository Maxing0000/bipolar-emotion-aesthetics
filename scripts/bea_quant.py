...
"""
BEA (Bipolar Emotion Aesthetics) 量化引擎 v2.12.0
...
def run_tests() -> bool:
    """运行自测试。实现在 tests/test_be.py（与引擎解耦，引擎文件保持精简）；
    本方法仅负责定位并委托执行，返回是否全部通过。"""
    root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    test_file = os.path.join(root, "tests", "test_be.py")
    if not os.path.exists(test_file):
        print("✗ 未找到自测文件 tests/test_be.py（请随技能包一并分发 tests/ 目录）")
        return False
    import subprocess
