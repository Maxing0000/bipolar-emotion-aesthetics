...
> 图片、调整建议、生成、灵敏度、批量等能力由 BEA 技能（SKILL.md）承载——技能侧可直接读图并按 `references/06-image-anchors.md` 打分，再把 t 值交给本 MCP 的 `bea_analyze` / `bea_compare` 算 W(T)。
...
起真实子进程走完整 MCP 会话（握手 → 工具列表 → 三工具调用 → 错误路径），共 22 项检查，并与引擎直算结果比对。
...
---
BEA v2.14.0 · 署名：马星 · CC BY-NC-SA 4.0
