...
def main():
    ...
    try:
        ...
        bad4 = call_tool(proc, "bea_compare",
                         {"category": "car", "a": "甲=1,2,3", "b": "乙=4,5,6"})
        ok(bad4.get("isError") is True, "紧凑格式维度数不足 → isError")
        bad5 = call_tool(proc, "bea_analyze",
                         {"category": "phone", "t_values": "形状=99,质感=6,色彩=4,构图=3,光影=5,细节=6"})
        ok(bad5.get("isError") is True, "t 值越界 → isError")
