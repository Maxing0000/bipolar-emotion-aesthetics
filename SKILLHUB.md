...
6. 填写信息：
   - **显示名称**：双极情绪美学 BEA
   - **版本号**：2.9.0（与 `manifest.json` 的 `version` 字段保持一致）
   - **描述**：见下方"描述模板"
7. 点击"发布"
...
2. 上传 `dist/bea-v2.9.0-skillhub.zip`——压缩包**根目录直接包含 `SKILL.md`**，符合官方"确认根目录包含 SKILL.md"的要求
...
> `bea-v2.9.0-full.zip` 内容与 skillhub 包相同，二者择一即可。
...
# 4. 正式发布
skillhub publish /Users/m/Documents/BEA --host https://api.skillhub.cn --changelog "v2.9.0 文档与打包规范优化"
```
...
| SkillHub | ✅ 已发布（v2.6.1，待同步 v2.9.0） | 从 GitHub 导入 |
